...
db_periodistas_menciones_mes_hito <- left_join(db_periodistas_menciones_mes, db_hitos, by = c("nombre_mencionado" = "NOMBRE")) |>
  mutate(fecha_flor = floor_date(Fecha, "month")) |>
...
grafica_06 <- ggplot(db_periodistas_menciones_mes_hito, aes(x = mes, y = nombre_mencionado, fill = countfactor)) +
  geom_tile(colour = "white", size = 0.25) + 
  geom_tile(data = subset(db_periodistas_menciones_mes_hito, !is.na(fecha_flor)), aes(x = fecha_flor, y = nombre_mencionado), colour = "black", size = 0.1) +
  labs(
  ...
  scale_fill_manual(values=c("#d53e4f", "#e75a48", "#f67a49", "#fba05b", "#fdc272", "#fee08b", "#f0ed93", "#daf09b", "#b7e2a2", "#c0e5ba", "#EAF7E8" ), na.value = "grey90")+
...
# Turn it interactive with ggplotly
grafica_06 <- ggplotly(grafica_06) |>
  layout(legend = list(orientation = "h", y = -0.3, yanchor = "top"))
grafica_06
