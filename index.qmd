...
db_periodistas_asesinados %>%
  datatable(options = list(pageLength = 85))
...
```{r grafica_01, fig.width = 10.5, fig.height = 5, dpi = 300, warning = FALSE, results = 'hide'}
...
grafica_01 <- ggplot(data = db_frecuencias_publicaciones, aes(x = `Post Created Date`)) + 
  ...
  labs(
    ...
    y = "Frecuencia",
    caption = "Publicaciones de páginas de Facebook con mencionando 'periodistas asesinado/a', 'reportero/a asesinado/a', 'comunicador/a asesinado/a', 'fotoreportero/a asesinado/a', 'corresponsal asesinado/a'",
  ) +
...
::: modal-content
```{r grafica_01_boton, fig.width = 10.5, fig.height = 5, dpi = 300, warning = FALSE, results = 'hide'}
...
### Frecuencias de publicaciones con asesinatos de periodistas

```{r grafica_02, fig.width = 10.5, fig.height = 5, dpi = 300, warning = FALSE, results = 'hide'}
...
::: {.modal-dialog .modal-dialog-centered .p-8 style="max-width: 90%"}
::: modal-content
```{r grafica_02_boton, fig.width = 10.5, fig.height = 5, dpi = 300, warning = FALSE, results = 'hide'}
...
```{r grafica_03, fig.width = 10.5, fig.height = 7, dpi = 300, warning = FALSE, results = 'hide'}
...
db_paises_top25 <- db_publicaciones_completas |>
...
# Gráfico de barra de publicaciones por país (grafica_03.1)
grafica_03.1 <- ggplot(db_paises_top25, aes(x = reorder(pais, -numero_publicaciones), y = numero_publicaciones)) +
  ...
  geom_text(data = subset(db_paises_top25, pais != "MX"), 
            aes(label = numero_publicaciones), vjust = -1.5, color = "black", size = 3) + 
  labs(title = NULL,
       x = NULL,
...
# Gráfica de porcentaje de publicaciones por país (grafica_03.2)
grafica_03.2 <- ggplot(db_paises_top25, aes(x = reorder(pais, -numero_publicaciones), y = porcentaje_publicaciones)) +
  geom_line(color = "darkred", group = 1) +
  geom_text(aes(label=ifelse(pais == "MX", sprintf("%.2f", porcentaje_publicaciones), "")), 
            vjust = 0, hjust = -0.25, color = "black", size = 3) +  
  geom_text(data = subset(db_paises_top25, pais != "MX"), 
            aes(label = sprintf("%.2f", porcentaje_publicaciones)), vjust = -1, color = "black", size = 3) + 
  labs(title = NULL,
       x = "País",
...
# Crear un textGrob para el título y caption de la grafica_03
titulo_grafica_03 <- textGrob("Publicaciones de páginas de Facebook por país", 
                              hjust = 0.5, vjust = 0, gp = gpar(fontsize = 16)
                              )

caption_grafica_03 <- textGrob("Publicaciones de páginas de Facebook con mencionando 'periodistas asesinado/a', 'reportero/a asesinado/a', 'comunicador/a asesinado/a', 'fotoreportero/a asesinado/a', 'corresponsal asesinado/a'",
                              gp = gpar(fontsize = 8)
                              )

# Gráfica de publicaciones por país (gráfica_03)
grafica_03 <- grid.arrange(
  titulo_grafica_03, grafica_03.1, grafica_03.2, caption_grafica_03, ncol = 1, nrow = 4, heights = c(0.25, 1, 1, 0.25)
)
...
::: {.modal-dialog .modal-dialog-centered .p-8 style="max-width: 90%"}
::: modal-content
```{r grafica_03_boton, fig.width = 10.5, fig.height = 7, dpi = 300, warning = FALSE, results = 'hide'}
...
# Creamos la variable que detecta las menciones de México
menciones_mexico <- db_publicaciones_completas |> 
  filter(str_detect(Message, regex("méx|mex", ignore_case = TRUE)) |
           str_detect(`Image Text`, regex("méx|mex", ignore_case = TRUE)) |
           str_detect(`Link Text`, regex("méx|mex", ignore_case = TRUE)) |
           str_detect(Description, regex("méx|mex", ignore_case = TRUE))) |> 
  group_by(`Page Admin Top Country`) |> 
  summarise(menciones_mexico = n())

# Creamos la base de datos que detecta las menciones de periodistas asesinados
# Crear una función para buscar nombres flexibles en las publicaciones
buscar_nombre_flexible <- function(nombre) {
  nombre <- tolower(nombre)  # Convertir el nombre a minúsculas para hacer la búsqueda insensible a mayúsculas y minúsculas
  db_publicaciones_completas %>%
    filter(str_detect(Message, regex(nombre, ignore_case = TRUE)) |
             str_detect(`Image Text`, regex(nombre, ignore_case = TRUE)) |
             str_detect(`Link Text`, regex(nombre, ignore_case = TRUE)) |
             str_detect(Description, regex(nombre, ignore_case = TRUE)))
}
...
for (nombre in db_periodistas_asesinados$Nombre) {
  ...
  menciones_por_pais$Nombre <- nombre  # Agregar el nombre del periodista
...
menciones_periodistas <- menciones_periodistas_por_pais %>%
  group_by(`Page Admin Top Country`) %>%
...
# Agregando la fila de totales al principio de db_paises
db_paises <- bind_rows(totales_paises, db_paises)
rm(totales_paises)

# Agregando las columnas de porcentaje con redondeo a dos decimales
db_paises <- db_paises |> 
  mutate(porcentaje_publicaciones = round((numero_publicaciones / total_publicaciones) * 100, digits = 2)) |> 
  select(`Page Admin Top Country`, numero_publicaciones, porcentaje_publicaciones, menciones_mexico, everything())

# Mostramos la tabla
kable(db_paises, format = "html", col.names = c("País", "Número de publicaciones", "%", "Número de menciones de México", "Número de menciones nombres de periodistas mexicanos asesinados/as"),
      caption = " Tabla 01: Publicaciones de páginas de Facebook por país y sus menciones de México y de nombres de periodistas asesinados/as") |> 
